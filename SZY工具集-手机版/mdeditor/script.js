(function () {
    ...
    btnExportTxt.addEventListener('click', function () {
        ...
        saveText(t, 'document.txt', 'text/plain;charset=utf-8');
        if (typeof plus === 'undefined') showToast('已导出 TXT');
    });
    btnExportMd.addEventListener('click', function () {
        var m = getMdContent();
        if (!m.trim()) { showToast('没有可导出的内容'); return; }
        saveText(m, 'document.md', 'text/markdown;charset=utf-8');
        if (typeof plus === 'undefined') showToast('已导出 MD');
    });
    btnExportHtml.addEventListener('click', function () {
        var h = getHtmlContent();
        if (!h.trim()) { showToast('没有可导出的内容'); return; }
        saveText(h, 'document.html', 'text/html;charset=utf-8');
        if (typeof plus === 'undefined') showToast('已导出 HTML');
    });
