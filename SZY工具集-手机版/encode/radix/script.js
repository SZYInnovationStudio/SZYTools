(function () {
    ...
    function convert() {
        ...
        try {
            var from = parseInt(fromBase.value), to = parseInt(toBase.value);
            var parts = val.split(/[\s,]+/).filter(Boolean);
            var results = [];
            for (var i = 0; i < parts.length; i++) {
                var num = parseInt(parts[i], from);
                if (isNaN(num)) { results.push('无效输入'); }
                else {
                    var converted = num.toString(to);
                    if (to === 16) converted = converted.toUpperCase();
