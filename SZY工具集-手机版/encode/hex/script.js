(function () {
    ...
    function getOverlay() {
        if (!overlayEl) { overlayEl = document.createElement('div'); overlayEl.className = 'sidebar-overlay'; overlayEl.addEventListener('click', closeSidebar); document.body.appendChild(overlayEl); }
        return overlayEl;
    }
    ...
    function switchMode(mode) {
        ...
        btnGoLabel.textContent = mode === 'encode' ? '编码' : '解码';
        outputLabel.textContent = mode === 'encode' ? 'Hex编码结果' : '解码结果';
        processText();
    }
    ...
    function processText() {
        ...
        try { txtOutput.value = curMode === 'encode' ? textToHex(val) : hexToText(val); }
        catch (e) { txtOutput.value = '输入有误: ' + e.message; }
    }

    txtInput.addEventListener('input', function () { charCount.textContent = txtInput.value.length + ' 字符'; processText(); });
    btnGo.addEventListener('click', function () { if (!txtInput.value.trim()) { txtInput.style.borderColor = '#fca5a5'; setTimeout(function () { txtInput.style.borderColor = ''; }, 1500); return; } processText(); outputGroup.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
    ...
    function copyToClipboard(text, message) {
        if (!text) return;
        if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(text).then(function () { showToast(message); }); }
        else { var ta = document.createElement('textarea'); ta.value = text; ta.style.cssText = 'position:fixed;left:-9999px;'; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); showToast(message); }
    }
